...
import { Play, Square, Music, Volume2, Sparkles, Wand2, CheckCircle, Sliders, Flame, AudioLines } from "lucide-react";
...
export default function SoundtrackStudio({ onSaveSoundtrack, savedSoundtrack }: SoundtrackStudioProps) {
  ...
  // New Live Audio FX params
  const [echoEnabled, setEchoEnabled] = useState(true);
  const [echoVolume, setEchoVolume] = useState(0.4); // feedback wet gain
  const [decayTime, setDecayTime] = useState(0.5); // note envelope decay multiplier

  // 8 steps, 5 pitch levels: C5(high), A4, G4, Fa4, C4(low)
  const pitchFrequencies = [523.25, 440.0, 392.0, 349.23, 261.63]; // C5, A4, G4, F4, C4
  const pitchNames = ["Do 5 (Céleste)", "La 4 (Sidéral)", "Sol 4 (Spatial)", "Fa 4 (Pulsar)", "Do 4 (Profond)"];
  ...
  // Preset harmonic loaders
  const loadChordProgression = (presetName: string) => {
    const clearGrid = Array(5).fill(null).map(() => Array(8).fill(false));
    if (presetName === "space-odyssey") {
      clearGrid[0][0] = true; clearGrid[1][1] = true; clearGrid[2][2] = true; clearGrid[3][3] = true;
      clearGrid[4][4] = true; clearGrid[0][5] = true; clearGrid[1][6] = true; clearGrid[2][7] = true;
    } else if (presetName === "nebula-heart") {
      clearGrid[4][0] = true; clearGrid[3][1] = true; clearGrid[1][2] = true; clearGrid[2][3] = true;
      clearGrid[4][4] = true; clearGrid[3][5] = true; clearGrid[0][6] = true; clearGrid[1][7] = true;
    } else if (presetName === "cyber-beat") {
      clearGrid[2][0] = true; clearGrid[2][2] = true; clearGrid[0][4] = true; clearGrid[2][6] = true;
      clearGrid[3][1] = true; clearGrid[3][3] = true; clearGrid[1][5] = true; clearGrid[3][7] = true;
    }
    setGrid(clearGrid);
    playTone(261.63, 0.45); // play pleasant tonic chord confirmation
  };
  ...
  const toggleCell = (row: number, col: number) => {
    ...
    setGrid(nextGrid);
    // Play note single shot for click feedback
    playTone(pitchFrequencies[row], 0.22);
  };

  // Upgraded physical Web Audio synthesis node chain with delay feedback
  const playTone = (frequency: number, duration: number) => {
    try {
      ...
      // Set waveform based on active instrument
      ...
      gainNode.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + duration * decayTime * 2.2);

      // Routing oscillator
      osc.connect(gainNode);

      if (echoEnabled) {
        // Wet echo feedback loop configuration
        delayNode.delayTime.setValueAtTime(0.32, ctx.currentTime); // pleasant 32ms tempo tap
        feedbackGain.gain.setValueAtTime(echoVolume, ctx.currentTime);
      ...
      osc.start();
      osc.stop(ctx.currentTime + duration * decayTime * 3.5);
    } catch (e) {
  ...
  useEffect(() => {
    if (isPlaying) {
      ...
      playIntervalRef.current = setInterval(() => {
        // Find active pitches for the step
  ...
  }, [isPlaying, grid, tempo, instrument, echoEnabled, echoVolume, decayTime]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-4">
            <span className="text-[10.5px] font-black text-slate-400 uppercase tracking-widest font-mono block">Réglages Acoustiques</span>

            {/* Instrument Selection */}
            <div className="space-y-1.5">
              <label className="text-[11px] text-slate-300 font-bold block">Oscillateur de Synthé</label>
              <select
                value={instrument}
                onChange={(e) => setInstrument(e.target.value)}
                className="w-full bg-slate-900 border border-slate-800 focus:border-violet-500 text-xs text-white px-3 py-2 rounded-xl focus:outline-none transition appearance-none font-medium"
              >
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center text-[11px]">
                <span className="text-slate-300 font-bold">Tempo (Vitesse)</span>
            ...
            <div className="space-y-1.5 pt-1.5 border-t border-slate-900">
              <div className="flex justify-between items-center text-[11px]">
                <label className="flex items-center gap-1.5 text-slate-300 font-bold">
                  ...
                  />
                  <span>Écho Feedback</span>
                </label>
                <span className="text-violet-400 font-mono font-bold">{Math.round(echoVolume * 100)}%</span>
              </div>
              <input
                type="range"
                min="0"
                ...
                value={Math.round(echoVolume * 100)}
                onChange={(e) => setEchoVolume(Number(e.target.value) / 100)}
                className="w-full accent-violet-500 cursor-pointer disabled:opacity-30"
              />
            </div>
            ...
            <div className="space-y-1.5 pt-1.5 border-t border-slate-900">
              <div className="flex justify-between items-center text-[11px]">
                <span className="text-slate-300 font-bold">Réverbération (Decay)</span>
                <span className="text-violet-400 font-mono font-bold">{Math.round(decayTime * 100)}%</span>
              </div>
              <input
                type="range"
                min="20"
                max="150"
                step="5"
                value={Math.round(decayTime * 100)}
                onChange={(e) => setDecayTime(Number(e.target.value) / 100)}
                className="w-full accent-violet-500 cursor-pointer"
              />
            </div>
          </div>

          {/* Magical Chords Arpeggio Selector */}
          <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 space-y-3">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest font-mono block">Arpèges Magiques</span>
            <div className="grid grid-cols-1 gap-1.5">
              <button
                type="button"
                onClick={() => loadChordProgression("space-odyssey")}
                className="text-left text-[11px] font-bold bg-slate-900 hover:bg-slate-850 text-violet-300 px-3 py-2 rounded-xl border border-violet-950/20 shadow-sm transition flex justify-between items-center cursor-pointer pointer-events-auto"
              >
                <span>🌌 L'Odyssée de l'Espace</span>
                <span className="text-[9px] font-mono text-slate-400">Classique</span>
              </button>
              <button
                type="button"
                onClick={() => loadChordProgression("nebula-heart")}
                className="text-left text-[11px] font-bold bg-slate-900 hover:bg-slate-850 text-pink-300 px-3 py-2 rounded-xl border border-pink-950/20 shadow-sm transition flex justify-between items-center cursor-pointer pointer-events-auto"
              >
                <span>🪐 Le Cœur de la Nébuleuse</span>
                <span className="text-[9px] font-mono text-slate-400">Mélancolique</span>
              </button>
              <button
                type="button"
                onClick={() => loadChordProgression("cyber-beat")}
                className="text-left text-[11px] font-bold bg-slate-900 hover:bg-slate-850 text-amber-300 px-3 py-2 rounded-xl border border-amber-950/20 shadow-sm transition flex justify-between items-center cursor-pointer pointer-events-auto"
              >
                <span>👾 Cyber Synth-Beat</span>
                <span className="text-[9px] font-mono text-slate-400">Électro Pro</span>
              </button>
            </div>
          </div>
